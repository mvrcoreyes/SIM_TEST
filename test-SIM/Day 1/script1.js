// Documento de calibración -- Parte 1
...
// Función para sumar los valores de calibración
function sumCalibrationValues(document) {
  // Dividir el documento en líneas individuales
  const lines = document.trim().split("\n");

  let totalSum = 0;

  for (let line of lines) {
    // Encontramos el primer y último dígito en cada línea
    const firstDigitMatch = line.match(/\d/);
    const lastDigitMatch = line.match(/\d(?=\D*$)/);

    // Verificar si se encontraron ambos dígitos y convertimos los dígitos encontrados a números
    if (firstDigitMatch && lastDigitMatch) {
      const firstDigit = parseInt(firstDigitMatch[0]);
      const lastDigit = parseInt(lastDigitMatch[0]);

      // Aqui sumamos los valores de la calibracion
...
const result = sumCalibrationValues(input1);
console.log(`PARTE 1`);
console.log(`La suma de todos los valores de calibración es:`, result);
